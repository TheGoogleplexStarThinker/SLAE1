...
#ifndef MY_PROJECT_C_FOR_CHEBYSHEV_HPP
#define MY_PROJECT_C_FOR_CHEBYSHEV_HPP
...
std::vector<T> ChebyshevRoots(std::pair<T, T> section, size_t PowOf2){
    double PolyOrder = std::pow(2, PowOf2);
    ...
    for (int i = 0; i < PowOf2; ++i) {
        currOrder = pow(2, double(i+1));
        next_idx.resize(currOrder);
        for(size_t j = 0; j < currOrder - 1; j+=2){
            next_idx[j] = idx[j/2];
...
//    }
    ...
    while(flag){
        flag = std::norm(r, normType NormType)
    }
};
...
#endif //MY_PROJECT_C_FOR_CHEBYSHEV_HPP
